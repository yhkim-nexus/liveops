...
export default function SegmentEditPage({
  ...
  useEffect(() => {
    if (audience && !initialized) {
      ...
      if (audience.filterExpression) {
        setQueryMode(true);
        setFilterExpression(audience.filterExpression);
      }
      if (audience.conditionGroups.length > 0) {
        setConditionGroups(audience.conditionGroups);
      }
      setInitialized(true);
    }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    updateMutation.mutate(
      ...
      { onSuccess: () => router.push(`/segments/${id}`) },
    );
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <h2 className="text-lg font-semibold">오디언스 편집</h2>
      </div>
